...
function main(timestamp) {
	if (previousTime+1/FPS < timestamp) {
		...
		// Display background image according to music
		if (bgTransparency != 0 || (lastBgTransparency<0.5 && lastBgTransparency>0.5 && bgTransparency == 0)) {
			if (bgImageBlur != 0) {
				makeImageTransparent(blurredImageData, bgImageWidth, bgImageHeight, bgTransparency);
				ctx.putImageData(blurredImageData, bgImageX, bgImageY);
			} else {
				if (imageData != null) {
					makeImageTransparent(imageData, bgImageWidth, bgImageHeight, bgTransparency);
					ctx.putImageData(imageData, bgImageX, bgImageY);
				} else {
...
function makeImageTransparent (image, w, h, transp) {
	let coeff = Math.round(255*transp);
	for (let i=0; i<h*w; i++) {
		image.data[4*i+3] = coeff;
	}
}
...
audioPlayer.onended = () => { audioButton.src = srcPlay; bgTransparency = 0; }
...
audioPlayer.ontimeupdate = () => {
	...
	if (currentTime < 80) {
		...
		else if (currentTime < 45) bgTransparency = ((currentTime-25)/21)**1.5;
...
function calculateBgPos () {
	if (bgImage.naturalHeight/Game.height > bgImage.naturalWidth/Game.width) {
		bgImageHeight = Game.height;
		bgImageWidth = bgImage.width*(Game.height/bgImage.naturalHeight);
		bgImageX = (Game.width - bgImageWidth) / 2;
		bgImageY = 0;
	} else {
		bgImageWidth = Game.width;
		bgImageHeight = bgImage.height*(bgImage.naturalWidth/Game.width);
		bgImageX = 0;
		bgImageY = (Game.height - bgImageHeight) / 2;
	}
	
	ctx.drawImage(bgImage, bgImageX, bgImageY, bgImageWidth, bgImageHeight);
	imageData = ctx.getImageData(bgImageX, bgImageY, bgImageWidth, bgImageHeight);
	ctx.clearRect(-20, -20, Game.width+40, Game.height+40);


	if (window.Worker) {
		if (imageData != null) {
			const myWorker = new Worker("public/js/blurring.js?v=" + new Date().getTime());
			
			myWorker.postMessage([imageData.data, imageData.width, imageData.height, 5]);

			myWorker.onmessage = function(e) {
				myWorker.terminate();
				
				let blurredData = e.data;
				if (blurredData.length == imageData.data.length) {
					let blurredDataArray = new Uint8ClampedArray(blurredData);
					blurredImageData = new ImageData(blurredDataArray, bgImageWidth, bgImageHeight);
				}
			}
		}
	} else {
